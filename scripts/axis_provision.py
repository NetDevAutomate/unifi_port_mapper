...
class AxisProvisioner:
        ...
        try:
            ...
            if r.status_code == 200:
                try:
                    data = r.json()
                    debug_log(
                        f'Device info response keys: {list(data.get("data", {}).get("propertyList", {}).keys()) if "data" in data else "N/A"}'
                    )
                    # Check if device has video capabilities (cameras always have ONVIF)
                    props = data.get('data', {}).get('propertyList', {})
                    if props.get('ProdType') in (
                        'Network Camera',
                        'PTZ Dome Camera',
                        'Dome Camera',
                    ):
            ...
            payload = {
                ...
                'params': {
                    'user': {
                        'name': username,
                        'password': password,
                        'privileges': {'admin': privilege == 'admin'},
                    }
                },
            }
            r = await client.post(
    ...
    async def configure_onvif_user(
        ...
        try:
            ...
            debug_log(
                f'Legacy pwdgrp API: status={r.status_code}, response={r.text[:100] if r.text else "empty"}'
            )
            if r.status_code == 200 and 'Error' not in r.text:
            ...
            return (
                False,
                f"ONVIF user '{username}' requires manual setup via web UI (API restricted on this firmware)",
            )
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not await self.check_connection(client):
                ...
                results['details'].append(
                    {'task': 'connect', 'success': False, 'message': 'Could not connect to device'}
                )
                return results
            ...
            for user in users:
                if user.name in existing_users:
                    ...
                    success, msg = await self.create_user(
                        client, user.name, user.password, user.role
                    )
                    results['details'].append(
                        {
                            'task': f'user:{user.name}',
            ...
            for onvif_user in onvif_users:
                if not onvif_supported:
                    ...
                    success, msg = await self.configure_onvif_user(
                        client, onvif_user.name, onvif_user.password
                    )
                    results['details'].append(
                        {
                            'task': f'onvif:{onvif_user.name}',
            ...
            if existing_mqtt:
                ...
                current_host = existing_mqtt.get(
                    'host', existing_mqtt.get('server', {}).get('host', '')
                )
                current_topic = existing_mqtt.get('basetopic', existing_mqtt.get('baseTopic', ''))
                expected_host = (
                    mqtt.broker.replace('mqtt://', '').replace('mqtts://', '').split(':')[0]
                )
            ...
            if not mqtt_configured:
                ...
                results['details'].append(
                    {'task': 'mqtt', 'success': success, 'message': msg, 'action': 'configure'}
                )
...
async def main():
    ...
    users = [
        UserConfig(u['name'], u['password'], u.get('role', 'administrator'))
        for u in config.get('users', [])
    ]
    onvif_users = [
        OnvifUser(u['name'], u['password']) for u in config.get('onvif', {}).get('users', [])
    ]
