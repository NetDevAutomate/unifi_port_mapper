...
def main():
    ...
    parser = argparse.ArgumentParser(description='UniFi API Client - Interface with the UniFi Controller API')
    ...
    parser.add_argument('--endpoint', default='devices',
                        choices=['devices', 'ports', 'clients', 'sites', 'stats', 'topology', 'custom'],
                        help='API endpoint to query')
    parser.add_argument('--custom-endpoint', help='Custom API endpoint path (when --endpoint=custom)')
    parser.add_argument('--output', '-o', help='Output file for the API response (JSON format)')
