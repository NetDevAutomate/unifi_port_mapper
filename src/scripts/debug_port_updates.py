...
def debug_device_configuration(api_client: UnifiApiClient, device_id: str) -> None:
    ...
    for endpoint_info in debug_info.get('api_endpoints_tried', []):
        ...
        print(f"  {status} {endpoint_info['endpoint']} (Status: {endpoint_info['status_code']})")
    ...
    for port in port_table[:5]:  # Show first 5 ports
        ...
        print(f"  Port {port_idx}: '{port_name}' (Up: {port_up}, Enabled: {port_enabled})")
...
def test_port_update(api_client: UnifiApiClient, device_id: str, port_idx: int, new_name: str) -> None:
    """Test updating a specific port name with detailed logging."""
    log.info(f"=== Testing Port Update: Device {device_id}, Port {port_idx} -> '{new_name}' ===")
    ...
    if success:
        ...
        verification_success = api_client.verify_port_update(device_id, port_idx, new_name, max_retries=5)
...
def list_devices(api_client: UnifiApiClient) -> None:
    ...
    for device in devices:
        device_id = device['id'][:24] + "..." if len(device['id']) > 24 else device['id']
        name = device['name'][:23] + "..." if len(device['name']) > 23 else device['name']
        model = device['model'][:18] + "..." if len(device['model']) > 18 else device['model']
        device_type = device['type'][:8] + "..." if len(device['type']) > 8 else device['type']
        ip = device['ip'][:13] + "..." if len(device['ip']) > 13 else device['ip']
        state = device['state']
        
        print(f"{device_id:<26} {name:<25} {model:<20} {device_type:<10} {ip:<15} {state}")
    
    print("\nTo debug a specific device, use: --device-id <DEVICE_ID>")


def test_port_updates(api_client: UnifiApiClient, device_id: str, port_updates: Dict[int, str]) -> None:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Debug UniFi port update persistence issues')
    ...
    parser.add_argument('--test-port', type=int, help='Port index to test update (optional)')
    parser.add_argument('--test-name', default='TEST-PORT', help='Test name for port update')
    parser.add_argument('--port-updates', help='JSON string of port updates (e.g., {"2": "Genos", "3": "PC"})')
    parser.add_argument('--list-device-ids', action='store_true', help='List all devices with their IDs and names')
