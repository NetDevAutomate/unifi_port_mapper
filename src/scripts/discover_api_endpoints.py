...
def test_endpoint(session: requests.Session, endpoint: str, method: str = 'GET', test_data: Dict[str, Any] = None) -> Dict[str, Any]:
...
def discover_endpoints(api_client: UnifiApiClient, device_id: str = None) -> None:
    ...
    for i, pattern in enumerate(all_patterns):
        ...
        print(f"{endpoint:<70} {'GET':<6} {status_str:<8} {available_str:<10} {size_str:<8}")
        ...
        if device_id and 'rest/device' in endpoint and result['available']:
            ...
            print(f"{endpoint:<70} {'PUT':<6} {put_status_str:<8} {put_available_str:<10} {'-':<8}")
    ...
    update_endpoints = [r for r in available_endpoints if 'rest/device' in r['endpoint'] and r['method'] in ['PUT', 'POST']]
    if update_endpoints:
...
def main():
    ...
    parser.add_argument('--device-id', help='Device ID to test device-specific endpoints')
