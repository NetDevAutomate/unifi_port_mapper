...
def update_with_full_context(api_client: UnifiApiClient, device_id: str, port_updates: Dict[int, str]) -> bool:
    ...
    log.info(f"Current device: {device_details.get('name', 'Unknown')} ({device_details.get('model', 'Unknown')})")
    ...
    additional_fields = [
        'config_network', 'ethernet_table', 'radio_table', 'switch_caps',
        'port_overrides', 'mgmt_network_id', 'outdoor_mode_override',
        'lcm_brightness_override', 'lcm_idle_timeout_override'
    ]
...
def verify_port_updates(api_client: UnifiApiClient, device_id: str, expected_updates: Dict[int, str], max_attempts: int = 10) -> bool:
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            device_details = api_client.get_device_details(api_client.site, device_id)
            if not device_details:
                log.warning(f"Failed to get device details for verification attempt {attempt + 1}")
                continue
            ...
            for port in port_table:
                ...
                if port_idx in expected_updates:
                    ...
                    if actual_name == expected_name:
                        ...
                        log.warning(f"✗ Port {port_idx} mismatch - expected: '{expected_name}', actual: '{actual_name}'")
                        all_verified = False
            
            if all_verified:
                log.info(f"🎉 All port updates verified successfully after {attempt + 1} attempts!")
                return True
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Advanced fix for UniFi port name persistence')
    ...
    if success:
        ...
        if verified:
            ...
            log.warning("Consider checking the UniFi UI manually or trying --force-restart")
            return 2
    else:
