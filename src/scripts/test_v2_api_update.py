                    ...
                    if "data" in data:
                        ...
                        if isinstance(data["data"], list) and len(data["data"]) > 0:
                            ...
                            if "port_table" in device:
                                print(
                                    f"Port table found with {len(device['port_table'])} ports"
                                )
                            else:
...
def update_device_via_v2_api(
    ...
    if (
        ...
        and "data" in device_data
        and len(device_data["data"]) > 0
    ):
            ...
            if response.status_code in [200, 201, 202]:
                ...
                if verification_data:
                    ...
                    if (
                        ...
                        and "data" in verification_data
                        and len(verification_data["data"]) > 0
                    ):
                        ...
                        for port in verify_port_table:
                            ...
                            if port_idx in port_updates:
                                ...
                                if actual_name == expected_name:
                                    log.info(
                                        f"✓ Port {port_idx} verified: '{actual_name}'"
                                    )
                                    return True
                                else:
                                    log.warning(
                                        f"✗ Port {port_idx} mismatch: expected '{expected_name}', got '{actual_name}'"
                                    )

                return True
            else:
                log.warning(
                    f"PUT failed with {response.status_code}: {response.text[:200]}"
                )

                # Try POST request
                response = api_client.session.post(
                    endpoint, json=update_data, timeout=10
                )
                ...
                if response.status_code in [200, 201, 202]:
                    ...
                    log.warning(
                        f"POST failed with {response.status_code}: {response.text[:200]}"
                    )
        ...
        for endpoint in command_endpoints:
            try:
                ...
                for cmd_data in command_formats:
                    response = api_client.session.post(
                        endpoint, json=cmd_data, timeout=10
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test V2 API updates for UniFi port names"
    )
