#!/usr/bin/env python3
"""
Firmware Security Advisor CLI tool for UniFi networks.

Analyzes firmware versions across devices to identify security
vulnerabilities, outdated firmware, and upgrade priorities.
"""
...
from unifi_mapper.advisors.firmware_advisor import FirmwareAdvisor
...
def main():
    parser = argparse.ArgumentParser(
        description='UniFi Firmware Security Advisor - Analyze firmware security posture'
    )
    ...
    parser.add_argument('--min-score', type=int, default=0,
                        help='Minimum security score to pass (default: 0)')
    ...
    try:
        ...
        # Run analysis
        advisor = FirmwareAdvisor(api_client, site)

        logger.info("Running firmware security analysis...")
        ...
        if args.json:
            ...
            output = advisor.generate_report(result)

        # Output report
        if args.output:
            with open(args.output, 'w') as f:
                f.write(output)
            logger.info(f"Firmware security report saved to: {args.output}")
        else:
            print(output)

        # Return code based on findings
        if result.critical_count > 0:
            logger.error(f"CRITICAL: {result.critical_count} devices have critical vulnerabilities!")
            return 2

        if result.security_score < args.min_score:
            logger.error(f"Security score {result.security_score} is below minimum {args.min_score}")
            return 1

        if result.security_update_count > 0:
            logger.warning(f"{result.security_update_count} devices need security updates")

        logger.info(f"Firmware analysis complete - Security score: {result.security_score}/100")
        return 0
