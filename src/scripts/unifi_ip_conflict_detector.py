...
def get_api_client(args) -> UnifiApiClient:
    ...
    verify_ssl = args.verify_ssl if args.verify_ssl is not None else os.environ.get("UNIFI_VERIFY_SSL", "false").lower() == "true"
    ...
    if not base_url:
        console.print("[bold red]Error:[/bold red] No UniFi Controller URL provided. Use --url or set UNIFI_URL environment variable.")
        sys.exit(1)
    
    if not api_token and not (username and password):
        console.print("[bold red]Error:[/bold red] No authentication credentials provided. Use --token or --username/--password or set environment variables.")
        sys.exit(1)
    ...
    if not api_client.login():
        console.print("[bold red]Error:[/bold red] Failed to authenticate with the UniFi Controller.")
        console.print("[yellow]Debug info:[/yellow] Using URL: {}, Site: {}, Auth method: {}".format(
            base_url, site, "API token" if api_token else "Username/password"
        ))
        console.print("[yellow]Tip:[/yellow] Check your credentials and make sure the UniFi Controller is accessible.")
        sys.exit(1)
...
def get_client_connection_info(client: Dict[str, Any], api_client: UnifiApiClient, site: str) -> Tuple[str, str]:
    """
...
def find_ip_conflicts(api_client: UnifiApiClient, site: str, include_historical: bool = False) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    with Progress(
        SpinnerColumn(),
        TextColumn("[bold blue]Fetching client data from UniFi Controller...[/bold blue]"),
        transient=True,
    ) as progress:
    ...
    conflicts = {ip: clients for ip, clients in ip_to_clients.items() if len(clients) > 1}
    
    return conflicts

def display_ip_conflicts(conflicts: Dict[str, List[Dict[str, Any]]], api_client: UnifiApiClient, site: str) -> None:
    """
    ...
    """
    if not conflicts:
        console.print(Panel("[bold green]No IP address conflicts found.[/bold green]", 
                           title="UniFi IP Conflict Detector", 
                           border_style="green"))
        return
    
    # Create a tree for displaying conflicts
    tree = Tree(f"[bold red]IP Conflicts Found: {len(conflicts)} conflicting IP addresses[/bold red]")
    
    # Sort conflicts by IP address
    sorted_ips = sorted(conflicts.keys(), key=lambda ip: [int(octet) for octet in ip.split('.')])
    
    # Add each conflict to the tree
    for ip in sorted_ips:
        clients = conflicts[ip]
        ip_branch = tree.add(f"[bold yellow]IP: {ip}[/bold yellow] ({len(clients)} devices)")
        ...
        for client in clients:
            ...
            # Get connection info
            device_info, port_info = get_client_connection_info(client, api_client, site)
    ...
    for ip in sorted_ips:
        clients = conflicts[ip]
        mac_addresses = ", ".join([format_mac_address(client.get("mac", "")) for client in clients])
        device_names = ", ".join([client.get("name", client.get("hostname", "Unknown")) for client in clients])
    ...
    for ip in sorted_ips:
        ...
        for client in clients:
            ...
            detailed_table.add_row(ip, client_name, mac_address, device_info, port_info, status_display)
...
def main():
    ...
    parser.add_argument("--url", help="UniFi Controller URL (e.g., https://unifi.local:8443)")
    ...
    parser.add_argument("--include-historical", action="store_true", help="Include historical clients (may include stale data)")
