#!/usr/bin/env python3
"""
Link Quality Monitor CLI tool for UniFi networks.

Monitors physical layer health including CRC errors, SFP diagnostics,
duplex mismatches, and link stability.
"""
...
from unifi_mapper.analyzers.link_quality import LinkQualityMonitor
...
def main():
    parser = argparse.ArgumentParser(
        description='UniFi Link Quality Monitor - Physical layer health analysis'
    )
    ...
    parser.add_argument('--sfp-only', action='store_true', help='Only analyze SFP modules')
    ...
    try:
        ...
        # Run analysis
        monitor = LinkQualityMonitor(api_client, site)

        logger.info("Running link quality analysis...")
        ...
        if args.output:
            with open(args.output, 'w') as f:
                f.write(output)
            logger.info(f"Link quality report saved to: {args.output}")
        else:
            print(output)

        # Return code based on findings
        if result.critical_count > 0:
            logger.warning(f"Found {result.critical_count} critical link quality issues!")
            return 1
        elif result.warning_count > 0:
            logger.warning(f"Found {result.warning_count} link quality warnings")

        logger.info("Link quality analysis complete")
        return 0
