...
def get_api_client(args) -> UnifiApiClient:
    ...
    log.debug(f"Authentication method: {'API token' if api_token else 'Username/password'}")
...
def search_clients(api_client: UnifiApiClient, search_term: str, site: str) -> List[Dict[str, Any]]:
    """
    ...
    for client in clients:
        ...
        if (search_term in client_name or 
            search_term in hostname or 
            search_term in mac or 
            search_term in ip):
...
def display_clients(clients: List[Dict[str, Any]], api_client: UnifiApiClient, site: str) -> None:
    """
    ...
    """
    if not clients:
        console.print(Panel("[bold yellow]No matching clients found.[/bold yellow]", 
    ...
    for client in clients:
        ...
        # Get status
        status = "[green]Connected[/green]" if client.get("is_wired", False) or client.get("is_guest", False) or client.get("_is_guest_by_uap", False) else "[red]Disconnected[/red]"
...
def main():
    ...
    parser.add_argument("search_term", help="Search term (client name, hostname, MAC address, or IP address)")
    ...
    parser.add_argument("--suppress-ssl-warnings", action="store_true", help="Suppress SSL certificate warnings")
    ...
    # Search for clients
    console.print(f"[bold]Searching for clients matching '[cyan]{args.search_term}[/cyan]'...[/bold]")
