#!/usr/bin/env python3
"""
QoS Configuration Validator CLI tool for UniFi networks.

Validates QoS settings including DSCP trust, queue configuration,
voice VLAN settings, and rate limiting.
"""
...
from unifi_mapper.validators.qos_validator import QoSValidator
...
def main():
    parser = argparse.ArgumentParser(
        description='UniFi QoS Validator - Validate Quality of Service configuration'
    )
    ...
    try:
        ...
        # Run validation
        validator = QoSValidator(api_client, site)

        logger.info("Running QoS configuration validation...")
        ...
        if args.output:
            with open(args.output, 'w') as f:
                f.write(output)
            logger.info(f"QoS validation report saved to: {args.output}")
        else:
            print(output)

        # Return code based on findings
        if not result.passed:
            logger.error(f"QoS validation FAILED - {result.critical_count} critical, {result.high_count} high issues")
            return 2

        if args.strict and (result.medium_count > 0 or result.low_count > 0):
            logger.warning(f"QoS validation warnings in strict mode: {result.medium_count} medium, {result.low_count} low")
            return 1

        logger.info("QoS validation passed")
        return 0
