#!/usr/bin/env python3
"""
Broadcast/Multicast Storm Detector CLI tool for UniFi networks.

Monitors for broadcast and multicast storms that can degrade network performance.
"""
...
from unifi_mapper.analyzers.storm_detector import StormDetector
...
def main():
    parser = argparse.ArgumentParser(
        description='UniFi Storm Detector - Detect broadcast/multicast storms'
    )
    ...
    # Detection options
    parser.add_argument('--broadcast-threshold', type=int, default=1000,
                        help='Broadcast packets/sec threshold (default: 1000)')
    parser.add_argument('--multicast-threshold', type=int, default=5000,
                        help='Multicast packets/sec threshold (default: 5000)')
    parser.add_argument('--output', help='Output file for detection report')
    ...
    try:
        ...
        # Run detection
        detector = StormDetector(api_client, site)
        detector.BROADCAST_THRESHOLD_PPS = args.broadcast_threshold
        detector.MULTICAST_THRESHOLD_PPS = args.multicast_threshold

        logger.info("Running storm detection analysis...")
        ...
        if args.output:
            with open(args.output, 'w') as f:
                f.write(output)
            logger.info(f"Storm detection report saved to: {args.output}")
        else:
            print(output)

        # Return code based on findings
        if result.active_storms:
            logger.error(f"ACTIVE STORM DETECTED on {len(result.active_storms)} ports!")
            return 2
        elif result.storm_events:
            logger.warning(f"Found {len(result.storm_events)} storm events")
            return 1

        logger.info("Storm detection complete - no storms detected")
        return 0
