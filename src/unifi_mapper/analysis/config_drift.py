...
async def snapshot_config(output_path: str = DEFAULT_CONFIG_PATH) -> dict:
    ...
    for d in devices:
        ...
        if 'port_table' in d:
            entry['config']['ports'] = [
                {k: p.get(k) for k in PORT_TRACKED if k in p} | {'port_idx': p.get('port_idx')}
        ...
        if dev_type == 'uap' and 'radio_table' in d:
            entry['config']['radios'] = [
                {k: r.get(k) for k in RADIO_TRACKED if k in r} for r in d['radio_table']
            ]
...
async def detect_drift(baseline_path: str = DEFAULT_CONFIG_PATH) -> dict:
    ...
    for dev in baseline['devices']:
        ...
        if not current:
            drifts.append(
                {'device': dev_name, 'type': 'MISSING', 'details': 'Device no longer found'}
            )
            continue
