...
class BrowserVerifier:
    ...
    async def login(self) -> bool:
        ...
        try:
            ...
            login_url = (
                f'{self.controller_url}/login'
                if self.controller_url.startswith('https')
                else f'https://{self.controller_url}/login'
            )
            await self.page.goto(login_url, wait_until='networkidle')

            # Wait for login form
            await self.page.wait_for_selector(
                'input[placeholder*="Username"], input[placeholder*="Email"]', timeout=10000
            )

            # Fill credentials
            await self.page.fill(
                'input[placeholder*="Username"], input[placeholder*="Email"]', self.username
            )
    ...
    async def verify_port_name_in_ui(
        ...
        for attempt in range(max_attempts):
            try:
                log.info(
                    f'Browser verification attempt {attempt + 1}/{max_attempts} for {device_name} Port {port_idx}'
                )
                ...
                device_selector = self.page.locator(
                    'input[placeholder*="Select"], input:has-text("Office")'
                )
                ...
                name_cell = port_row.locator('td').nth(
                    3
                )  # Assuming Name is 4th column (0-indexed)
                actual_name = await name_cell.inner_text()

                log.info(
                    f"Browser UI shows Port {port_idx} name: '{actual_name}' (expected: '{expected_name}')"
                )
                ...
                if actual_name.strip() == expected_name.strip():
                    ...
                    log.warning(
                        f"❌ Browser verification FAILED: Port {port_idx} expected '{expected_name}', got '{actual_name}'"
                    )
...
def verify_ports_with_browser(
    controller_url: str, username: str, password: str, verifications: List[Dict[str, any]]
) -> Dict[str, Tuple[bool, str]]:
