...
class DeviceDefinition:
    ...
    """
    def __init__(self, name: str, port_count: int, port_naming_scheme: str = "LAN {}",
                 special_ports: Dict[int, str] = None, sfp_ports: List[int] = None):
        """
...
DEVICE_DEFINITIONS = {
    ...
    "u6-mesh-pro": DeviceDefinition(
        name="UniFi AP 6 Mesh Pro",
        port_count=1,
        port_naming_scheme="Main Port"
    ),
...
def get_device_definition(model: str) -> Optional[DeviceDefinition]:
    ...
    if model_lower.startswith("usw") or model_lower.startswith("us-") or model_lower.startswith("usl"):
        # It's a switch
    ...
    elif model_lower.startswith("uap") or model_lower.startswith("u6") or "ap" in model_lower:
        ...
        elif "pro" in model_lower and not "mesh" in model_lower:
