...
class GroundTruthVerifier:
    ...
    def verify_port_updates_ground_truth(
        self, device_updates: Dict[str, Dict[int, str]], use_browser: bool = False
    ...
    def _verify_with_enhanced_api_checks(
        self, device_updates: Dict[str, Dict[int, str]]
    ) -> Dict[str, Dict[int, bool]]:
        ...
        for device_id, port_updates in device_updates.items():
            ...
            for port_idx, expected_name in port_updates.items():
                ...
                if consistency_results['consistent'] and consistency_results['matches_expected']:
                    ...
                    log.info(
                        f"✅ Port {port_idx}: Verified '{expected_name}' (consistent across reads)"
                    )
                else:
                    device_results[port_idx] = False
                    log.error(
                        f"❌ Port {port_idx}: Expected '{expected_name}', "
    ...
    def _verify_with_browser(
        ...
        try:
            ...
            log.warning(
                'Browser verification not implemented yet - falling back to enhanced API checks'
            )
            return self._verify_with_enhanced_api_checks(device_updates)
        ...
        for device_id, port_updates in device_updates.items():
            ...
            for port_idx, expected_name in port_updates.items():
                ...
                if verification_success:
                    ...
                    report_lines.append(
                        f"  ❌ Port {port_idx}: Expected '{expected_name}' - FAILED"
                    )
...
def verify_with_ground_truth(
    ...
    report = verifier.generate_verification_report(device_updates, results)
