...
class InferredNetworkTopology(NetworkTopology):
    ...
    def infer_connections(self) -> List[Dict[str, Any]]:
        ...
        device_types = {"router": [], "switch": [], "ap": [], "unknown": []}

        for device_id, device in self.devices.items():
            device_dict = {"id": device_id, "name": device.name, "model": device.model}
            ...
            if main_router:
                ...
                if not core_switches:
                    for switch in switches:
                        ...
                        if "usw" in model and (
                            "lite" in model or "pro" in model or "ultra" in model
                        ):
                            core_switches.append(switch)
    ...
    def generate_report(self, filename: str) -> bool:
        ...
        try:
            with open(filename, "w") as f:
                ...
                for device_id, device in self.devices.items():
                    device_dict = {"name": device.name, "model": device.model}
                    device_type = self.determine_device_type(device_dict)

                    f.write(
                        f"| {device.name} | {device.model} | {device_type} | {device.mac} |\n"
                    )
                ...
                f.write("\n## Notes\n\n")
                f.write(
                    "* This network topology is inferred based on device types and naming conventions.\n"
                )
                f.write(
                    "* Actual connections may differ from the inferred connections shown in the diagram.\n"
                )
                f.write(
                    "* Device types are determined based on model names and device names.\n"
                )
                f.write(
                    "* Connections are color-coded by device type: routers (blue), switches (green), access points (red).\n"
                )
                f.write(
                    "* The Dream Machine Pro Max is shown as the central node in the network.\n"
                )
    ...
    def generate_mermaid_diagram(self) -> str:
        ...
        for device_id, device in self.devices.items():
            device_dict = {"name": device.name, "model": device.model}
            if (
                device.name == "Dream Machine Pro Max"
                or self.determine_device_type(device_dict) == "router"
            ):
                router = device
                router_id = device_id
                break
        ...
        for device_id, device in self.devices.items():
            ...
            mermaid.append(
                f'    {device_id}["{device.name}"] --> |{device.model}| {device_id}_type[{device_type}]'
            )
        ...
        if router:
            router_id = (
                "dream_machine_pro_max"
                if router.name == "Dream Machine Pro Max"
                else router_id
            )
        ...
        for connection in self.connections:
            ...
            if connection.get("inferred", False):
                mermaid.append(
                    f'    {source_id} -- "Inferred Connection" --> {target_id}'
                )
            else:
                mermaid.append(
                    f'    {source_id} -- "{source_port} -> {target_port}" --> {target_id}'
                )
...
def generate_inferred_topology(
    base_url=None,
    site="default",
    api_token=None,
    username=None,
