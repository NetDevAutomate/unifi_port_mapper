...
class ProtectMonitor:
    ...
    def get_bootstrap(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            resp = self.session.get(f'{self.base_url}/proxy/protect/api/bootstrap', timeout=15)
            if resp.status_code == 200:
                return resp.json()
        except Exception as e:
    ...
    def print_ai_port_details(self, ap_state: Dict[str, Any]) -> None:
        ...
        print(
            f'  Firmware: {ap_state["firmware"]} | Connection Host: {ap_state["connectionHost"]}'
        )
    ...
    def print_camera_details(self, cam_state: Dict[str, Any]) -> None:
        ...
        for ch in cam_state['channels']:
            if ch.get('enabled'):
                print(f'     {ch["name"]}: {ch["width"]}x{ch["height"]} @ {ch["fps"]}fps')
        ...
        try:
            while True:
                ...
                for cam in bootstrap.get('cameras', []):
                    ...
                    if '192.168.10' in str(cam.get('host', '')) or cam.get('isThirdPartyCamera'):
