...
from .models import (
    DeviceInfo, PortInfo, NetworkHealthStatus, PortHealthMetrics, 
    DeviceHealthMetrics, NetworkTopologyChange, NetworkConfiguration,
...
class NetworkAnalyzer:
    ...
    def perform_comprehensive_analysis(self, devices: List[DeviceInfo]) -> NetworkAnalysisResult:
        """
        ...
        try:
            ...
            self.configuration_history = [
                config for config in self.configuration_history
            ...
            # Security analysis
            security_issues = self._analyze_security_posture(devices, result.device_health)
            ...
            log.info(f"Analysis completed: {result.total_devices} devices analyzed, "
    ...
    def _analyze_device_health(self, device: DeviceInfo) -> Optional[DeviceHealthMetrics]:
        """Analyze health metrics for a single device."""
        try:
            # Get detailed device information including system metrics
            device_details = self.api_client.get_device_details(self.api_client.site, device.id)
            ...
            if system_stats:
                ...
                health.temperature_celsius = system_stats.get('temps', {}).get('Board', 0.0)
                
            # Extract uptime
            health.uptime_seconds = device_details.get('uptime', 0)
            health.firmware_version = device_details.get('version', 'unknown')
            health.last_seen = datetime.fromtimestamp(device_details.get('last_seen', 0))
    ...
    def _analyze_port_health(self, device_id: str, port_idx: int, port_data: Dict[str, Any]) -> Optional[PortHealthMetrics]:
        ...
        try:
            ...
            if speed_bps > 0 and metrics.uptime_seconds > 0:
                ...
                metrics.utilization_percent = min(100.0, (total_bytes / max_theoretical_bytes) * 100)
            
            # Link stability indicators
            metrics.uptime_seconds = port_data.get('uptime', 0)
            if port_data.get('up', False):
                metrics.last_state_change = datetime.now() - timedelta(seconds=metrics.uptime_seconds)
            
            # Estimate link flaps from uptime patterns (heuristic)
            if metrics.uptime_seconds < 86400:  # Less than 24 hours uptime suggests recent changes
                metrics.link_flap_count = 1
            
            return metrics
            
        except Exception as e:
            log.debug(f"Error analyzing port health for device {device_id}, port {port_idx}: {e}")
            return None
    
    def _build_topology_snapshot(self, devices: List[DeviceInfo]) -> NetworkConfiguration:
        ...
        for device in devices:
            ...
            device_details = self.api_client.get_device_details(self.api_client.site, device.id)
            if device_details:
                config.add_device_config(device.id, {
                    'name': device.name,
                    'model': device.model,
                ...
                if port.has_lldp_info and port.connected_device_name:
                    ...
                    if connected_device:
                        ...
                        if connected_port_idx:
                            config.add_connection(device.id, port.idx, connected_device.id, connected_port_idx)
        
        return config
    
    def _detect_topology_changes(self, current_config: NetworkConfiguration) -> List[NetworkTopologyChange]:
        ...
        for device_info in comparison['devices_added']:
            changes.append(NetworkTopologyChange(
                'device_added',
    ...
    def _analyze_configuration_drift(self, current_config: NetworkConfiguration) -> Dict[str, Any]:
        ...
        drift_analysis = {
            'devices_changed': len(comparison['devices_added']) + len(comparison['devices_removed']),
            'topology_changes': len(comparison['connections_added']) + len(comparison['connections_removed']),
            'configuration_stability_score': self._calculate_stability_score(comparison),
            'period_analyzed_hours': (current_config.timestamp - baseline.timestamp).total_seconds() / 3600,
    ...
    def _identify_performance_bottlenecks(self, device_health: Dict[str, DeviceHealthMetrics]) -> List[Dict[str, Any]]:
        ...
        for device_id, health in device_health.items():
            ...
            if health.cpu_usage_percent > 80:
                bottlenecks.append({
                    'type': 'high_cpu_usage',
                    'device_id': device_id,
                    'severity': 'critical' if health.cpu_usage_percent > 95 else 'warning',
            ...
            if health.memory_usage_percent > 85:
                bottlenecks.append({
                    ...
                    'device_id': device_id,
                    'severity': 'critical' if health.memory_usage_percent > 95 else 'warning',
            ...
            if health.temperature_celsius > 70:
                bottlenecks.append({
                    ...
                    'device_id': device_id,
                    'severity': 'critical' if health.temperature_celsius > 80 else 'warning',
            ...
            for port_idx, port_metrics in health.port_metrics.items():
                if port_metrics.utilization_percent > 85:
                    bottlenecks.append({
                        ...
                        'device_id': device_id,
                        'port_idx': port_idx,
                        'severity': 'critical' if port_metrics.utilization_percent > 95 else 'warning',
                ...
                if port_metrics.rx_errors + port_metrics.tx_errors > 1000:
                    bottlenecks.append({
                        ...
                        'metric_value': port_metrics.rx_errors + port_metrics.tx_errors
                    })
        
        return bottlenecks
    
    def _analyze_security_posture(self, devices: List[DeviceInfo], device_health: Dict[str, DeviceHealthMetrics]) -> List[Dict[str, Any]]:
        ...
        for device in devices:
            ...
            if device_health_metrics.last_seen:
                hours_since_seen = (datetime.now() - device_health_metrics.last_seen).total_seconds() / 3600
                if hours_since_seen > 24:
                    security_issues.append({
                        'type': 'device_offline',
                        'device_id': device.id,
                        'severity': 'critical' if hours_since_seen > 72 else 'warning',
    ...
    def _generate_recommendations(self, analysis_result: NetworkAnalysisResult) -> List[Dict[str, Any]]:
        """Generate improvement recommendations based on analysis."""
        recommendations = []
        
        # Health-based recommendations
        critical_devices = [
            device_id for device_id, health in analysis_result.device_health.items()
        ...
        if analysis_result.performance_bottlenecks:
            ...
            if high_priority_bottlenecks:
                recommendations.append({
                    ...
                    'details': [b['description'] for b in high_priority_bottlenecks]
                })
        
        # Configuration drift recommendations
        if analysis_result.configuration_drift:
            stability_score = analysis_result.configuration_drift.get('configuration_stability_score', 100)
        ...
        if critical_security_issues:
            recommendations.append({
                ...
                'details': [issue['description'] for issue in critical_security_issues]
            })
    ...
    def save_analysis_result(self, result: NetworkAnalysisResult, output_dir: Path):
        ...
        try:
            ...
            output_file = output_dir / f"network_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
