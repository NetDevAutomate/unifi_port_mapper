...
class UnifiPortMapper:
    ...
    def update_port_name(self, device_id: str, port_idx: int, new_name: str, verify_update: bool = True) -> bool:
        """
    ...
    def get_client_port_mapping(self, device_mac: str) -> Dict[int, List[Dict[str, Any]]]:
        ...
        for client in clients_data:
            ...
            if (client.get('is_wired', False) and
                client.get('sw_mac', '').lower() == device_mac.lower()):
                ...
                if is_online and is_active:
                    ...
                    if port_idx is not None:
                        ...
                        log.debug(f"Found ACTIVE client '{client_name}' (online={is_online}, last_seen={client.get('last_seen', 0)}) on port {port_idx}")
    ...
    def batch_update_port_names(self, device_id: str, port_updates: Dict[int, str], verify_updates: bool = True) -> bool:
        """
        ...
        if verify_updates:
            ...
            if verification_failures:
                log.error(f"Port name verification failed for {len(verification_failures)} ports on device {device_id}:")
                for port_idx, expected_name in verification_failures:
                    log.error(f"  Port {port_idx}: Expected '{expected_name}' but verification failed")
                return False
            else:
                log.info(f"All {len(port_updates)} port name updates verified successfully for device {device_id}")
