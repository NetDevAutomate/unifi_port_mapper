...
def generate_port_mapping_report(devices: Dict[str, DeviceInfo], output_path: str) -> None:
    """
    ...
    for device_id, device in devices.items():
        ...
        # Add port table
        report.append("| Port | Status | Current Name | Proposed Name | Connected Device | PoE | Modified |")
        report.append("|------|--------|--------------|---------------|------------------|-----|----------|")
        ...
        for port in device.ports:
            ...
            # Add the port row
            report.append(f"| {port.idx} | {status} | {current_name} | {proposed_name} | {connected_device} | {poe_status} | {modified} |")
