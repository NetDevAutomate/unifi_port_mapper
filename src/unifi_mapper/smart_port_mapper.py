...
class SmartPortMapper:
    ...
    def smart_update_ports(
        ...
        for device in devices_data:
            ...
            capability = self.capability_detector.detect_capabilities(
            ...
            strategy = self.capability_detector.get_recommended_strategy(
                device_model, firmware_version
            )
    ...
    def _apply_device_aware_updates(
        ...
        if capability.port_naming_support == PortNamingSupport.RESETS_AUTOMATICALLY:
            log.warning(
                f'⚠️  {device_name}: Device auto-resets configurations - update likely to fail'
            )
    ...
    def _perform_ground_truth_verification(self, results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def generate_smart_mapping_report(self, results: Dict[str, Any]) -> str:
        ...
        if results['verification_failures']:
            ...
            for failure in results['verification_failures']:
                report_lines.append(
                    f"   • {failure['device_name']} Port {failure['port_idx']}: Expected '{failure['expected_name']}'"
                )
        ...
        if summary['skipped_incompatible'] > 0:
            report_lines.append(
                f'• {summary["skipped_incompatible"]} devices have known port naming limitations'
            )
            report_lines.append('  Consider manual UI-based configuration for these devices')

        if summary['failed_verification'] > 0:
            report_lines.append(
                f'• {summary["failed_verification"]} port updates failed verification'
            )
            report_lines.append(
                '  These devices may need firmware updates or manual configuration'
            )

        if summary['successful_updates'] > 0:
            report_lines.append(
                f'• {summary["successful_updates"]} port updates verified successfully'
            )
